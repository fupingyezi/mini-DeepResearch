...
3. 按照标准研究报告格式组织内容，包括但不限于以下结构元素（根据问题需要灵活调整）：
引言（问题背景与目标）
方法论（如有建模、实验或计算）
分析与结果（含图表、公式、代码块等必要支撑）
讨论（对结果的解释、局限性、与其他工作的对比）
结论（总结核心发现与建议）
4. 对需要技术细节的部分，应使用规范格式呈现：
数学公式用LaTeX格式（如$E = mc^2$或独立公式环境）；
代码需放入可读性强的代码块中，并附简要说明；
数据或引用应注明来源（若子任务中提供）。

输出要求：仅返回最终研究报告正文，不包含任何元信息、系统提示、JSON 结构、额外注释或自我指涉语句。语言应准确、简洁、专业，符合学术或行业报告规范。`;
